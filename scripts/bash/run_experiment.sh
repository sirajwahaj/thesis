...
if [[ -x "$REPO_ROOT/src/.venv/Scripts/python" ]]; then
  ...
  PYTHON="python3"
  echo "[WARN] src/.venv not found — run: cd src && uv sync"
fi
...
for level in "${LEVELS[@]}"; do
  ...
  for rep in $(seq 1 "$REPETITIONS"); do
    ...
    if [[ "$ENVIRONMENT" == "vm" ]]; then
      python3 "$SCRIPTS_DIR/collect_vm_metrics.py" \
        --output "${RUN_DIR}/vm_metrics.csv" &
      METRICS_PID=$!
    else
      bash "$SCRIPTS_DIR/bash/collect_k8s_metrics.sh" \
        --output "${RUN_DIR}/k8s_pod_metrics.csv" &
      METRICS_PID=$!
    fi

    # Trigger concurrent runs with level and rep tags for traceability
    python3 "$SCRIPTS_DIR/trigger_dagster_runs.py" "$level" \
      --host "$DAGSTER_HOST" \
      --port "$DAGSTER_PORT" \
      --level "$level" \
      --rep "$rep" \
    ...
    # On K8s, all concurrent jobs run in parallel so wait is fixed: startup(~40s) + workload + buffer.
    # On VM, jobs may queue so scale by level count.
    if [[ "$ENVIRONMENT" == "k8s" ]]; then
      WAIT_TIME=$((WORKLOAD_SECS + 90))
    else
    ...
    # PostgreSQL is never directly exposed — tunnel/port-forward as needed
    PG_HOST="localhost"
    PG_PORT="15432"
    PG_FWD_PID=""
