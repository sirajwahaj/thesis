...
if [[ "$CHECK_VM" == "true" ]]; then
  ...
  if [[ ! -f "$VM_IP_FILE" ]]; then
    ERRORS+=("VM: vm-ip.txt not found — run: make bootstrap")
    echo "  [FAIL] vm-ip.txt: NOT FOUND"
  else
    VM_IP="$(cat "$VM_IP_FILE" | tr -d '[:space:]')"
    if [[ -z "$VM_IP" ]]; then
      ERRORS+=("VM: vm-ip.txt is empty — run: make bootstrap")
      ...
      if ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
        ...
        # 3. gRPC server on port 4000
        if ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no \
            "ubuntu@$VM_IP" "nc -z localhost 4000" &>/dev/null; then
          echo "  [OK] gRPC server: listening on port 4000"
        else
          ERRORS+=("VM: gRPC server not running on port 4000 — run: make vm-provision")
          echo "  [FAIL] gRPC server: NOT RUNNING on port 4000"
        fi
        # 4. Systemd service
        SERVICE_STATE="$(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no \
            "ubuntu@$VM_IP" "systemctl is-active thesis-workload" 2>/dev/null || echo "unknown")"
        if [[ "$SERVICE_STATE" == "active" ]]; then
          echo "  [OK] systemd service 'thesis-workload': active"
        else
          WARNINGS+=("VM: thesis-workload service state: $SERVICE_STATE (expected: active)")
          echo "  [WARN]  systemd service 'thesis-workload': $SERVICE_STATE"
        fi
      else
