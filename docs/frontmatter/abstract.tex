...
The VM's job success rate degraded sharply with concurrency: 100\,\% at L1--L2, falling to 66.7\,\% at L3, 46.7\,\% at L5, 42.9\,\% at L7, and 30.0\,\% at L10. The failure mechanism was memory exhaustion: three or more concurrent containers each allocating 400\,MB exceeded the kernel's memory budget on the 4\,GB VM, triggering Linux OOM kills. Execution time for surviving jobs remained relatively flat (65--80\,s) across all levels, demonstrating that OOM-kill acts as a filter rather than a performance degradation mechanism. Kubernetes maintained 100\,\% success across all concurrency levels via per-pod cgroup limits, with consistent execution time and low variance. Pod scheduling latency was under 1\,second; container startup overhead ranged from 4.3\,s at L1 to 14.9\,s at L6 in the local Kind environment with pre-pulled images, growing with concurrency due to concurrent initialisation contention. The reliability crossover --- the first concurrency level where VM success rate drops below 95\,\% while Kubernetes maintains 100\,\% --- occurred at \textbf{L3 (3 concurrent jobs)}. These findings give engineering teams an empirical, hardware-specific framework for migration decisions.
