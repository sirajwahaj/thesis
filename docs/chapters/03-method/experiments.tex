...
\paragraph{Procedure.} Run all six workload levels (L1, L2, L3, L4, L5, L6). Each level is repeated 3 times. Record: job success rate, mean execution time, execution time standard deviation, CPU utilisation, and memory utilisation.
...
\begin{description}[style=nextline]
  ...
  Run identical workload levels (L1, L2, L3, L4, L5, L6) as Experiment~1, 3 repetitions each. Record same metrics.

  \item[Part B --- Blast radius test (folded in)]
  At level L4 (5 concurrent jobs), under organic concurrent load, measure whether resource-contention failures cascade to sibling jobs. Record:
  \begin{enumerate}[label=(\alph*)]
    \item success rate of all concurrent jobs (VM vs.\ K8s),
    \item whether failures in one job affect siblings (blast radius binary: 0=contained, 1=cascades).
  \end{enumerate}

  \item[Part C --- Spike observation (folded in)]
  At L6 (10 concurrent jobs, exceeding single-node CPU capacity by 2.5\,$\times$), observe and record: how many jobs are delayed pending scheduling, how long until all jobs begin executing, any job failures due to scheduling timeout. This provides supplementary high-load data without a dedicated experiment.
\end{description}
