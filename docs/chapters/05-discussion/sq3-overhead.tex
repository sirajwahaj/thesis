...
At L1 (1 job), startup overhead was 4.3\,s; at L2 it was 4.6\,s; at L3, 5.1\,s; at L4 (5 jobs), 6.8\,s; at L5 (7 jobs), 11.6\,s; and at L6 (10 jobs), 14.9\,s.
...
At L6 (10 concurrent jobs), the 14.9\,s startup overhead represents $\approx$18\% of the 96.5\,s total K8s pod time.
At L1, the 4.3\,s overhead is $\approx$7\% of the 67.7\,s total.
...
The K8s startup overhead is substantially higher (4--15\,s) due to the additional Kubernetes control plane handshake, gRPC code server connection, and PostgreSQL connection establishment per pod.
...
In this experiment, the scheduling component is negligible ($<$1\,s); the startup overhead ranges from 4.3\,s to 14.9\,s depending on concurrency.
For the tested 60-second workload, K8s adds 7--18\% overhead compared to raw execution time --- meaningful but outweighed by the reliability benefits at L3+.

\paragraph{Overhead grows with concurrency but remains bounded.}
Container startup overhead increased from 4.3\,s at L1 to 14.9\,s at L6, growing with the number of concurrent pods.
